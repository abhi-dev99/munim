...
**Version:** 1.0  
**project:** Kleos 4.0 — D.Y. Patil RAIT, Navi Mumbai  
...
| Component | Technology | Purpose |
|---|---|---|
| Local tunnel | **ngrok** | Expose local FastAPI to Meta webhook (project) |
| Deployment | **server** (free $5 credit) or local + ngrok | FastAPI + n8n containers |
...
- **project cost:** <500 messages total → < $0.01 → effectively ₹0
- **Setup:** upstash.com → Create Kafka cluster → Get bootstrap URL + credentials
- **Note:** Use single-zone (cheaper); multi-zone unnecessary for project

### Upstash Redis
- **Free tier:** 500,000 commands/month, 256MB data, 10GB bandwidth
- **project usage:** GSTIN cache hits, session state → well under 500K commands
- **Cost:** ₹0
...
- **project strategy:** Cache ALL results in Redis permanently. Pre-seed ~30 supplier GSTINs at startup. Live-verify only genuinely new GSTINs during demo. Mock the rest.
- **Expected cost:** < ₹50 for entire project
...
- **Deployment:** Not confirmed. If cloud (server), update `NGROK_URL` to server domain. If local, ngrok is sufficient.
