# munim Device Capability Utilization Spec — Munim.ai / Pune City Battle

**Doc type:** Technical feature catalog + prioritization (PRD/SRS hybrid)
**Scope:** Every credible way to use munim-provided hardware/tooling (Snapdragon NPU, camera, mic, Office Kit) inside Munim's FinTech & Commerce track submission
**Constraints this spec is written against:** team of 3 students, 3× Windows laptops, 3× munim loaner phones (one per member) issued at check-in, 30-hour window (Sat 08:00 check-in → Sun ~17:00 awards), extending the existing Munim.ai codebase (not a from-scratch build), track = FinTech and Commerce (city-battle only, drops at Finale)
...
| **`PRODUCT`** | A real feature an actual trader/CA would use after the project — must survive the question "would this exist if HackTracker didn't exist?" | Jury: end product (30%), novelty (20%), technical depth (15%) |
...
**VOI-2 · On-device TTS for the Hindi diagnosis** — read the existing `diagnosis_hi` text aloud via the phone's built-in on-device TTS instead of (or alongside) the WhatsApp text bubble. → Directly serves literacy-accessibility for the exact user this product targets, and is nearly free to build (TTS engines are OS-native, no model integration needed) — a very high value-to-effort ratio. → **S** → `PRODUCT` + `SCORE` (voice/on-device AI, arguably the cheapest genuine on-device AI touchpoint in this entire catalog) → **BUILD — cheapest real win here, do this even if nothing else voice-related lands.**
  - **Status: BUILT** (branch `munim/on-device-tts`). `frontend/src/app/utils/speech.js` wraps `window.speechSynthesis`; `frontend/src/app/components/ListenButton.js` is the Listen/Stop control; wired into the post-upload scan-result toast in `frontend/src/app/trader/page.js`. Not yet wired into invoice history / `InvoiceDetailModal.js` — good follow-up, same utility already covers it.

**VOI-3 · Voice-note context capture** — trader adds a short voice memo alongside a photo ("cash payment, Ramesh Traders") transcribed on-device to enrich extraction context (e.g. payment method for the 180-day reversal rule). → Nice-to-have, addresses a real gap (payment status is currently manual), but adds a new data-capture UX mid-project with no existing hook to hang it on. → **M–L** → `PRODUCT` → **catalog.**

### OFK — Office Kit (build-time usage, not a product dependency)

Be explicit about this distinction in the pitch: **Office Kit is a project build tool provided by munim, not something a real trader's CA has installed.** Do not present any of these as end-user product features — that's a fabricated claim a technical judge will catch instantly. All OFK items are pure `SCORE`.
...
**NPU-1 · On-device semantic HSN match** — replace/front the existing pgvector semantic-fallback pass in `hsn.py` with a small on-device sentence-embedding model (ONNX, NPU-accelerated) for the *first* pass, falling back to the cloud pgvector path only on low-confidence. → This is the one idea in the whole catalog that satisfies "local model at the core" without being a bolt-on gimmick — it accelerates and localizes a real, already-existing, already-correct engine component (`hsn.py`'s two-pass exact+semantic design), rather than adding a cosmetic feature next to the real product. → **L** (swapping a model into a working pipeline under time pressure is genuinely risky) → `PRODUCT` + `SCORE` → **catalog unless the Saturday teach-in reveals munim-provided tooling that makes this trivial (they mention free AI credits + NPU-targeted tooling at check-in — reassess after that).**
...
- **"Office Kit as a product feature"** — rejected on principle, see § OFK above. Office Kit is munim's dev bridge, not something a trader's CA has installed; presenting it as end-user functionality is a claim a judge will immediately see through.
