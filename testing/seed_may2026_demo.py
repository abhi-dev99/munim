"""
Munim.ai — Demo Seeder: Sept 2026 Test Data
==========================================
...
SUPPLIERS = [
    ...
    },
    {
        ...
        "is_einvoice_mandated": False,
        "health_score": 52,  # Deducted for NOT filing Sept 2026
    },
    {
    ...
    },
    {
        ...
        "taxpayer_type": "Regular",
        "registration_date": "2026-09-14",  # <180 days — fraud signal!
...
set_flag(vtech_id, "NEW_ENTITY_HIGH_VALUE", {"gstin_age_days": 100, "invoice_value": 476000, "registration_date": "2026-09-14"})
...
GSTR2B_RECORDS = [
    ...
    {"supplier_gstin": "27AABCN5432P1Z3", "invoice_number": "NOL/2025-26/1847", "invoice_date": "2026-09-08", "taxable_value": 22500.00, "cgst": 1350.00, "sgst": 1350.00, "igst": 0.00, "itc_eligible": True},
    # Scenario 2 — fuzzy match (typo: missing leading zeros)
    {"supplier_gstin": "33AADCS7891K1Z2", "invoice_number": "SVLC/MAY/2026/892", "invoice_date": "2026-09-14", "taxable_value": 19000.00, "cgst": 0.00, "sgst": 0.00, "igst": 2280.00, "itc_eligible": True},
    ...
    {"supplier_gstin": "27AADCW3456P1Z8", "invoice_number": "WR/MAY/2026/4521", "invoice_date": "2026-09-22", "taxable_value": 3500.00, "cgst": 315.00, "sgst": 315.00, "igst": 0.00, "itc_eligible": True},
    # Scenario 5 — Shree Vinayak (filed with wrong HSN rate)
    {"supplier_gstin": "27ABCPV6789H1Z1", "invoice_number": "SVSW/2526/344", "invoice_date": "2026-09-05", "taxable_value": 24000.00, "cgst": 2160.00, "sgst": 2160.00, "igst": 0.00, "itc_eligible": True},
    # Scenario 6 — VisionTech (new GSTIN, fraud risk)
    {"supplier_gstin": "27AADCV3421P1ZN", "invoice_number": "VTOS/2026/001", "invoice_date": "2026-09-25", "taxable_value": 425000.00, "cgst": 25500.00, "sgst": 25500.00, "igst": 0.00, "itc_eligible": True},
    # Scenario 7 — Titan (different invoice number format)
    {"supplier_gstin": "29AABCT8765P1Z4", "invoice_number": "TITANB00040320", "invoice_date": "2026-09-12", "taxable_value": 36000.00, "cgst": 0.00, "sgst": 0.00, "igst": 4320.00, "itc_eligible": True},
    # Scenario 8 — Arihant (MISSED ITC — filed in 2B, never scanned by trader)
    {"supplier_gstin": "27AABCA9876H1Z7", "invoice_number": "AOL/2526/0847", "invoice_date": "2026-09-02", "taxable_value": 38000.00, "cgst": 2280.00, "sgst": 2280.00, "igst": 0.00, "itc_eligible": True},
]

print("\n━━━ STEP 4: Inserting GSTR-2B records (month=5, year=2026) ━━━")

# First clear existing Sept 2026 records for this trader
db.table("gstr2b_records").delete().eq("trader_id", TRADER_ID).eq("month", 5).eq("year", 2026).execute()
print("  – Cleared existing Sept 2026 GSTR-2B records")
...
INVOICES = [
    {
        ...
        "invoice_hash": make_hash("27AABCN5432P1Z3", "NOL/2025-26/1847", "2026-09-08"),
        "processed_at": now_ts,
    },
    {
        "trader_id": TRADER_ID,
        ...
        "invoice_hash": make_hash("33AADCS7891K1Z2", "SVLC/MAY/2026/00892", "2026-09-14"),
        "processed_at": now_ts,
    },
    {
        "trader_id": TRADER_ID,
        ...
        "itc_amount_blocked": 0.00,
        "itc_block_reason": "Invoice not found in GSTR-2B — Royal Optica Frames has not filed GSTR-1 for Sept 2026",
        "gstr2b_match_status": "ITC_AT_RISK",
        "gstr2b_match_confidence": 0.0,
        "fraud_score": 12,
        "fraud_signals": {"signals": []},
        "processing_duration_ms": 1950,
        "invoice_hash": make_hash("24AABCR8765H1Z4", "ROF/2526/MAY/0156", "2026-09-19"),
        "processed_at": now_ts,
    },
    {
        "trader_id": TRADER_ID,
        ...
        "invoice_hash": make_hash("27AADCW3456P1Z8", "WR/MAY/2026/4521", "2026-09-22"),
        "processed_at": now_ts,
    },
    {
        "trader_id": TRADER_ID,
        ...
        "invoice_hash": make_hash("27ABCPV6789H1Z1", "SVSW/2526/344", "2026-09-05"),
        "processed_at": now_ts,
    },
    {
        "trader_id": TRADER_ID,
        ...
        "invoice_hash": make_hash("27AADCV3421P1ZN", "VTOS/2026/001", "2026-09-25"),
        "processed_at": now_ts,
    },
    {
        "trader_id": TRADER_ID,
        ...
        "invoice_hash": make_hash("29AABCT8765P1Z4", "TET/NAS/0512/2026", "2026-09-12"),
        "processed_at": now_ts,
    },
]
...
print(f"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  DEMO SEED COMPLETE — Sept 2026, Suryakant Optics
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
