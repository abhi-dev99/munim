...
# The wrong-identity guard itself (exact-canonical-wins, ambiguous-fallback-
# refuses) lives in services/db.py now -- resolve_trader_rows(column,
# canonical, variants) -- so both the Supabase and DynamoDB backends share
# identical, tested semantics instead of duplicating this logic per backend.
...
async def request_otp(data: OTPRequest):
    ...
    try:
        ...
        # one that was typed -- but never across two different traders, see
        # db.resolve_trader_rows.
        res_trader_data = await db.resolve_trader_rows("whatsapp_number", phone, variants)
        res_ca_data = await db.resolve_trader_rows("ca_whatsapp_number", phone, variants)
    except Exception as e:
        logger.error(f"DB Error: {e}")
        return GENERIC_RESPONSE

    if not res_trader_data and not res_ca_data:
        return GENERIC_RESPONSE
    ...
    lang = "en"
    if res_trader_data:
        lang = res_trader_data[0].get("language_pref") or "en"
    elif res_ca_data:
        lang = res_ca_data[0].get("language_pref") or "en"
    ...
    try:
        ...
        if res_ca_data:
            ...
            if not trader:
                ...
                trader = res_ca_data[0] if res_ca_data else None
    except Exception as e:
