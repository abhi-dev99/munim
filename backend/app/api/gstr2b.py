    ...
    for rec in payload.records:
        try:
            ...
            if rec.record_type == "B2BA":
                await db.delete_gstr2b_b2b_record(payload.trader_id, supplier_gstin, invoice_number)
            await db.upsert_gstr2b_record(
                payload.trader_id, payload.month, payload.year, supplier_gstin, invoice_number, fields
            )
            inserted += 1
...
async def upload_gstr2b_file(
    ...
    for rec in records:
        try:
            ...
            fields = {k: v for k, v in rec.items() if k not in ("supplier_gstin", "invoice_number")}
            # B2BA amendments supersede the original — delete old B2B row first
            if rec.get("record_type") == "B2BA":
                await db.delete_gstr2b_b2b_record(trader_id, supplier_gstin, invoice_number)
            await db.upsert_gstr2b_record(trader_id, month, year, supplier_gstin, invoice_number, fields)
            inserted += 1
        except Exception as e:
...
async def get_gstr2b_records(trader_id: str = Depends(verify_trader_access), month: int = None, year: int = None):
    ...
    try:
        records = await db.get_gstr2b_records(trader_id, month, year)
        records = sorted(records, key=lambda r: r.get("invoice_date") or "", reverse=True)
        return {
            "records": records,
            "total": len(records),
        }
...
async def get_missed_itc_snapshot(
    ...
    try:
        ...
        if not month or not year:
            all_records = await db.get_gstr2b_records(trader_id)
            if all_records:
                latest = max(all_records, key=lambda r: (r.get("year") or 0, r.get("month") or 0))
                month = month or latest.get("month")
        ...
        period_records = await db.get_gstr2b_records(trader_id, month, year)
        unmatched = [r for r in period_records if not r.get("matched_invoice_id")]
        total_2b = len(period_records)
        ...
        linked_count = sum(1 for r in period_records if r.get("matched_invoice_id"))
        reconciled = linked_count > 0

        gstins = {r.get("supplier_gstin") for r in unmatched if r.get("supplier_gstin")}
        names: dict[str, str] = {}
        for gstin in gstins:
            sup = await db.get_supplier_by_gstin(gstin)
            if sup:
                names[gstin] = sup.get("trade_name") or sup.get("legal_name") or ""
...
async def clear_gstr2b_records(month: int, year: int, trader_id: str = Depends(verify_trader_access)):
    ...
    try:
        await db.delete_gstr2b_records_for_period(trader_id, month, year)
...
async def trigger_reconciliation(trader_id: str = Depends(verify_trader_access), month: int = None, year: int = None):
    ...
    try:
        ...
        try:
            ...
            if trader:
                auto_warn_vendors = trader.get("auto_warn_vendors", False)
        except Exception as e:
        ...
        for period_month, period_year in _adjacent_periods(month, year):
            for rec in await db.get_gstr2b_records(trader_id, period_month, period_year):
        ...
        for inv in unmatched:
            ...
            await db.update_invoice_by_id(inv["id"], {
            ...
            if is_matched:
                ...
                if match_result.matched_record_id:
                    await db.mark_gstr2b_record_matched(match_result.matched_record_id, inv["id"])
            else:
        ...
        for upd in cn_updates:
            try:
                existing = await db.get_invoice_by_id_for_trader(upd["invoice_id"], trader_id)
                if not existing:
                    cn_failed += 1
                    logger.warning(f"Credit note target invoice {upd['invoice_id']} not found")
                    continue
                current_itc = float(existing.get("itc_amount_eligible") or 0)
                new_itc = max(0.0, current_itc + upd["itc_delta"])
                await db.update_invoice_by_id(upd["invoice_id"], {
