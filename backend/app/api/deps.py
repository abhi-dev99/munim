...
async def get_practice_client_ids(current_trader_id: str) -> list[str]:
    ...
    me = await db.get_trader_by_id(current_trader_id)
    if not me:
        return []

    ids = {current_trader_id}
    variants = _phone_variants(me.get("whatsapp_number", ""))
    if variants:
        clients = await db.resolve_trader_rows("ca_whatsapp_number", variants[0], variants)
        ids.update(c["id"] for c in clients if c.get("id"))
    return sorted(ids)
...
async def verify_trader_access(trader_id: str, current_trader_id: str = Depends(get_current_trader_id)) -> str:
    ...
    user = await db.get_trader_by_id(current_trader_id)
    if not user:
    ...
    # being equal means.
    variants = _phone_variants(user.get("whatsapp_number", ""))
    if not variants:
    ...
    clients = await db.resolve_trader_rows("ca_whatsapp_number", variants[0], variants)
    if not any(c.get("id") == trader_id for c in clients):
