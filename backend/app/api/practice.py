...
async def practice_overview(current_trader_id: str = Depends(get_current_trader_id)):
    ...
    try:
        ...
        # why. Supabase round trips dominate this endpoint on that backend,
        # so an earlier version ran these three reads concurrently via
        # asyncio.to_thread. They all share one process-wide `supabase-py`
        # client, whose underlying httpx connection pool is synchronous and
        # multiplexes over HTTP/2 -- driving it from three threads at once
        # produced `RemoteProtocolError: Server disconnected` on all three
        # reads simultaneously, intermittently. Intermittently is the worst
        # kind: it passed twice before it failed.
        #
        ...
        # so this runs one read at a time. (DynamoDB's boto3 client doesn't
        # share connections this way, but this stays sequential on both
        # backends rather than diverging in concurrency behaviour.)
        traders = await db.get_traders_by_ids(client_ids)
        invoices = await db.get_invoices_for_traders(client_ids)
        gstr2b = await db.get_gstr2b_records_for_traders(client_ids)
...
async def client_brief(trader_id: str, current_trader_id: str = Depends(get_current_trader_id)):
    ...
    try:
        ...
        invoices = [i for i in invoices if i.get("itc_status") in _OPEN_STATUSES]
