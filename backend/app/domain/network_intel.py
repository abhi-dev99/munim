...
async def _fetch_network_rows(gstins: list[str]) -> list[dict]:
    """
    ...
    return await db.get_invoices_by_gstin_suppliers(gstins)


async def bulk_network_signals(
    gstins: Iterable[str],
    ...
    rows = await _fetch_network_rows(wanted)
...
async def supplier_network_signal(
    gstin: str,
    requesting_trader_id: Optional[str] = None,
) -> dict:
    ...
    result = await bulk_network_signals([gstin], requesting_trader_id)
    return result.get(gstin, _empty("No network data for this GSTIN."))
