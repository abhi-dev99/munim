...
// See src/app/page.js's identical line for why -- this dashboard now runs
// on AWS Lambda + DynamoDB when built with that env var set. Note: two
// sub-features reachable from here (vendor fix-link copying in
// PracticePanel, and the Gemini-keys admin modal -- currently unreachable
// dead code anyway, no trigger button renders it) still point at whatever
// backend API_BASE resolves to, but their endpoints (vendor.py, admin.py)
// are not mounted on the AWS Lambda -- known, deliberately deferred gap.
