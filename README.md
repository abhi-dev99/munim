...
2. Go to [server.app](https://server.app) → New Project → Deploy from GitHub → Select `munim-ai` → Set **Root Directory** to `backend/`
3. Add a Redis service in server dashboard
4. Copy server's auto-generated Redis URL and set `UPSTASH_REDIS_URL` in your server env vars
5. Set all other env vars (Gemini key, Supabase, Meta tokens) in server → Variables
6. server auto-deploys on every push to `main`

**Your backend URL** will be: `https://your-app.up.server.app`

### Frontend → server
1. Go to [server.com](https://server.com) → New Project → Import `munim-ai` → Set **Root Directory** to `frontend/`
2. Add env var: `NEXT_PUBLIC_API_URL=https://your-app.up.server.app`
3. Deploy — done in 2 minutes

### WhatsApp Webhook Setup
1. In Meta Developer Console → Your App → WhatsApp → Configuration
2. Set Webhook URL: `https://your-app.up.server.app/api/v1/webhook`
...
Trader (WhatsApp) → Meta Cloud API
                    ...
                    └── Fraud Scorer (6 signals)
                          ↓
                    Supabase DB → CA Dashboard (Next.js/server)
