...
| Email | Cloudmailin |
| Cache | Redis (Upstash) |
| Deploy | server (backend) + server (frontend) |
...
- **Backend → server**: Root dir `backend/`, add Redis service, set env vars
- **Frontend → server**: Root dir `frontend/`, set `NEXT_PUBLIC_API_URL`
